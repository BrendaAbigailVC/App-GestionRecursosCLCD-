...
const FilaTabla = styled.tr``; const CuerpoTabla = styled.tbody``; const Celda = styled.td` padding: 10px; border-bottom: 1px solid #ddd; text-align: center; `;
...
const HeaderSeccion = styled.div` background: #5d9cec; color: white; padding: 14px; font-weight: 600; cursor: pointer; display: flex; justify-content: space-between; align-items: center; border-radius: 6px; margin-top: 10px; opacity: ${props => props.bloqueado ? 0.6 : 1}; pointer-events: ${props => props.bloqueado ? "none" : "auto"}; `;
...
const InputCantidad = styled.input` width: 30px; height: 30px; text-align: center; border: 0.5px solid #ccc; border-radius: 6px; font-weight: 600; box-sizing: border-box; &::-webkit-outer-spin-button, &::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; } -moz-appearance: textfield; `;
const LayoutPrincipal = styled.div` display: grid; grid-template-columns: 1fr 260px; gap: 1px; width: 90%; margin: auto; `;
...
const RegistrarPrestamo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [resMateriales, resAlumnos, resEmpleados, resEmpleado] =
          await Promise.all([
            fetch(`${API_BASE_URL}/materiales`, { headers }),
            fetch(`${API_BASE_URL}/alumnos`, { headers }),
            fetch(`${API_BASE_URL}/empleados`, { headers }),
            fetch(`${API_BASE_URL}/empleados/perfil/${keycloak.tokenParsed.sub}`, { headers }),
          ]);

        const empleadoData = await resEmpleado.json();
        setEmpleado(empleadoData);

        const resContador = await fetch( `${API_BASE_URL}/prestamos/contador/${empleadoData.noeconomico}`, { headers } );

        const { contador } = await resContador.json();

        const consecutivo = String(contador + 1).padStart(6, "0");

        setIdPrestamo( `LABPRES-${empleadoData.noeconomico}-${consecutivo}` );
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/prestamos/prestamo`, {
        ...
        body: JSON.stringify({
          ...
          idEmpleado: empleado.noeconomico,
          estadoPrestamo: 0,
          ...datosPrestamo,
            ...
            <ContenedorAcordeon>
              ...
              <ContenidoSeccion abierto={seccionActiva === "Solicitante"}>
                ...
                {busquedaSolicitante && !solicitante && (
                  <div
                    style={{ backgroundColor: "#fff", border: "1px solid #ccc", borderRadius: "5px", maxHeight: "150px", overflowY: "auto", marginBottom: "1rem", }}
                  >
                              ...
                              <FilaTabla key={m.id}>
                                ...
                                <Celda>
                                  <ControlCantidad>
                                    ...
                                    {m.tipo === 1 ? (
                                      <InputCantidad type="number" min="0" max={m.cantidad} value={carrito[m.id] || 0} onChange={(e) => cambiarCantidad(m.id, parseInt(e.target.value) || 0) } />
                                    ) : (
                                      <NumeroCantidad>
                                        {carrito[m.id] || 0}
                                      </NumeroCantidad>
                                    ...
                                    <BotonCantidad type="button" onClick={() => aumentarCantidad(m.id)} disabled={(carrito[m.id] || 0) >= m.cantidad} >
                                      +
                                    </BotonCantidad>
                                  </ControlCantidad>
                                </Celda>
              ...
              <SeccionBloque bloqueado={!hayMaterial}>
                ...
                <ContenidoSeccion abierto={seccionActiva === "datos"}>
                  ...
                  {hayMaterial && (
                    <>
                      No. Económico
                      <Input2 type="text" value={empleado?.noeconomico} disabled />
                      ID Préstamo
                      ...
                      Fecha inicio
                      <Input2 type="date" name="fechaPrestamo" value={datosPrestamo.fechaPrestamo} disabled />
                      Fecha devolución
                      <Input2 type="date" name="fechaDevolucion" value={datosPrestamo.fechaDevolucion} onChange={(e) => setDatosPrestamo({ ...datosPrestamo, fechaDevolucion: e.target.value, }) } disabled={datosPrestamo.tipoPrestamo === "0"} error={erroresMensaje.fechaDevolucion} />
                      <MensajeConError error={erroresMensaje.fechaDevolucion} />
                      UEA
                      <Input2 type="text" inputMode="numeric" pattern="[0-9]*" name="uea" value={datosPrestamo.uea} onChange={(e) => { const value = e.target.value; if (/^\d*$/.test(value)) { setDatosPrestamo({ ...datosPrestamo, uea: value }); } }} error={erroresMensaje.uea} />
                      <MensajeConError error={erroresMensaje.uea} />
                      Grupo
                      <Input2 type="text" name="grupo" value={datosPrestamo.grupo} onChange={(e) => setDatosPrestamo({ ...datosPrestamo, grupo: e.target.value }) } error={erroresMensaje.grupo} />
      ...
      {mostrarVistaPrevia && (
        <ModalFondo>
          <ModalContenido>
            ...
            >
              <Boton as="button" onClick={() => setMostrarVistaPrevia(false)}> Cancelar </Boton>
              <Boton as="button" primario onClick={(e) => { e.preventDefault(); confirmarPrestamo(); }} > Confirmar </Boton>
            </div>
          </ModalContenido>
        </ModalFondo>
      )}
