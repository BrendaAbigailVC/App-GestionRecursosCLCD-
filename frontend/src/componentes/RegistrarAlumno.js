...
const RegistrarAlumno = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      //creamos el objeto limpio para el backend
      const datosParaEnviar = {
        matricula: formData.matricula,
        id_keycloak: null, //Se vinculará automáticamente al primer login
        nombre: formData.nombre,
        apellidopaterno: formData.apellidop,
        apellidomaterno: formData.apellidom,
        unidad: parseInt(formData.unidad),
        division: formData.division,
        licenciatura: parseInt(formData.licenciatura),
      ...
      const response = await fetch("http://localhost:4001/api/alumnos", {//cambio de estandar
        method: "POST",
      ...
      if (!response.ok) {
         // Si el backend responde con error (400, 500), lanzamos el error para el catch
