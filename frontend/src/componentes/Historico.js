...
const Historico = () => {
  ...
  useEffect(() => {
    // 2. Nueva validación basada en Keycloak
    if (initialized) {
  ...
  return (
    <>
      <Helmet><title>Mostrar Préstamos</title></Helmet>
      <Header>
        <ContenedorHeader><Titulo>Histórico</Titulo></ContenedorHeader>
      </Header>
      ...
      <ContenedorBusqueda>
        ...
        />
        <SelectFiltro value={filtroColor} onChange={(e) => setFiltroColor(e.target.value)}>
        ...
        <CuerpoTabla>
          {prestamosFiltrados.map((p) => {
            const tieneIncidencias =
              p.estadoprestamo === 1 &&
              p.observaciones &&
              p.observaciones.split("|").some((obs) => !obs.toLowerCase().includes("sin incidencias"));
            ...
            return (
              <FilaTabla key={p.id}>
                ...
                <Celda>
                  <div>
                    <strong>{p.solicitante_tipo === "ALUMNO" ? "Matrícula" : "No. Económico"}:</strong> {p.solicitante_codigo}
                  </div>
                  <div>{p.solicitante_nombre}</div>
                </Celda>
                <Celda>{p.empleado_nombre}</Celda>
                <CeldaEstado color={color}>
                  {fechaDevolucion ? fechaDevolucion.toLocaleDateString("es-MX") : "Sin fecha"}
                </CeldaEstado>
                ...
                <CeldaEstado
                  color={p.estadoprestamo === 0 ? "gray" : tieneIncidencias ? "orange" : "green"}
                >
                  {p.estadoprestamo === 0 ? "Pendiente" : tieneIncidencias ? "Con incidencias" : "Sin incidencias"}
                </CeldaEstado>
                <Celda>
                  <Boton onClick={() => navigate(`/mostrar-prestamo/${p.id}`)}>Mostrar Más</Boton>
                </Celda>
              </FilaTabla>
            );
          })}
