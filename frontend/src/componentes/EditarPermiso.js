...
const EditarPermiso = () => {
  ...
  useEffect(() => {
    const fetchEmpleado = async () => {
      const res = await fetch(`http://148.206.162.62:4000/empleado/${id}`);
      const data = await res.json();
      setEmpleado(data);
    };

    const fetchPermisosDisponibles = async () => {
      const res = await fetch("http://148.206.162.62:4000/permisos-empleado");
      const data = await res.json();
      setPermisosDisponibles(data);
    };

    const fetchPermisosAsignados = async () => {
      const res = await fetch(`http://148.206.162.62:4000/empleado/${id}/permisos`);
      const data = await res.json();
